...
try{
      _counter++;
  await FirebaseFirestore.instance.collection('game').doc(docId).update({'teamPts1':_counter});
  setState(() {
    });
}catch (e){
...
try{
  ...
  try{
      _time2++;
  await FirebaseFirestore.instance.collection('game').doc(docId).update({'teamPts2':_time2});
  setState(() {
    });
}catch (e){
  throw CustomSnackBar(message: 'Ocorreu um erro', ref: ref);
...
await FirebaseFirestore.instance.collection('game').doc(argModality).update({'winner':data['team1']});
 await FirebaseFirestore.instance
      .collection('game')
        .where("modalidade", isEqualTo: data['modalidade']).where('id', isEqualTo: data['nextGame'])
      .get().then((value) => value.docs.first.id).then((value) {
        nextgameDocId = value;
      });
 await FirebaseFirestore.instance
      .collection('game')
        .where("modalidade", isEqualTo: data['modalidade']).where('id', isEqualTo: data['nextGame'])
      .get().then((value) => value.docs.first.data()).then((value) {
        nextGameData = value;
      });
print("opa");
...
await FirebaseFirestore.instance.collection('game').doc(nextgameDocId).update({'team1':data['team1']});
}else{
await FirebaseFirestore.instance.collection('game').doc(nextgameDocId).update({'team2':data['team1']});
}
...
await FirebaseFirestore.instance.collection('game').doc(argModality).update({'winner':data['team2']});
 await FirebaseFirestore.instance
      ...
      if(nextGameData!['team1'] == null){
await FirebaseFirestore.instance.collection('game').doc(nextgameDocId).update({'team1':data['team2']});
}else{
await FirebaseFirestore.instance.collection('game').doc(nextgameDocId).update({'team2':data['team2']});
